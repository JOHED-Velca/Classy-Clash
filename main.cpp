...
int main()
{
    ...
    while (!WindowShouldClose())
    {
        ...
        if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON))
        {
            for (auto enemy : enemies)
            {
                if (CheckCollisionRecs(enemy->getCollisionRec(), knight.getWaeaponCollisionRec()))
                {
                    enemy->setAlive(false); //format Windows: shift+alt+f Linux:ctrl+shift+i Mac:shift+ops
                }
            }
        }
        else
